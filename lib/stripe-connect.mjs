...
} from './stripe-cashflow.mjs';

const STRIPE_ENV_KEYS = STRIPE_READ_ONLY_ENV_KEYS;
...
export function getStripeConnectConfigStatus(env = process.env) {
  ...
  return {
    ...
    warning: missingKeys.length
      ? 'Stripe keys or account IDs are not configured; dashboard remains in safe mock mode.'
      : 'Stripe keys and account IDs are present, but live onboarding remains disabled by guardrail.',
    ...
    setupRequest: missingKeys.length
      ? `Add these production env vars in the dashboard hosting environment: ${missingKeys.join(', ')}. Use a Stripe restricted secret key with read-only Balance, Balance transactions, and Accounts permissions only.`
      : 'All required Stripe env vars are present for read-only reporting.',
  };
}
